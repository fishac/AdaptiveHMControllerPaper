#ifndef LINEARCONSTANTINCOMPLETECONTROLLER_DEFINED__
#define LINEARCONSTANTINCOMPLETECONTROLLER_DEFINED__

#include "Controller.hpp"

using namespace arma;

class LinearConstantIncompleteController : public Controller {
public:
	double Hnm1_pow;
	double Hn_pow;
	...
	LinearConstantIncompleteController(double P_, double p_, double tol_, double safety_factor_, double k1_[2], double k2_[1], Controller* initial_controller_) {
		name = "LinearConstantIncomplete";
		P = P_;
		p = p_;
		tol = tol_;
	...
	void update_exponent_terms() {
		...
		M_essn_pow = (k1[0]+k1[1])*(p+1.0)/(2.0*P*p);
		M_esfnm1_pow = k2[0]/p;
		M_esfn_pow = -k2[0]/p;
	}
