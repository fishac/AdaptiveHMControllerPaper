#ifndef LINEARCONSTANTCONTROLLER_DEFINED__
#define LINEARCONSTANTCONTROLLER_DEFINED__

#include "Controller.hpp"

using namespace arma;

class LinearConstantController : public Controller {
public:
	double Hnm1_pow;
	double Hn_pow;
	double H_essnm1_pow;
	...
	LinearConstantController(double P_, double p_, double tol_, double safety_factor_, double k1_[2], double k2_[2], Controller* initial_controller_) {
		name = "LinearConstant";
		P = P_;
		p = p_;
		tol = tol_;
	...
	double get_new_H() {
		...
		if (iteration == 1) {
			...
			double H_terms = std::pow(Hnm1,Hnm1_pow)*std::pow(Hn,Hn_pow);
