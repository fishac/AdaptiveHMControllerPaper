				...
				for (int l=0; l<n_esf_mt; l++) {
					if (!has_failed) {
						...
						if (ret.status == 0) {
							...
							//objective_function_value += 100.0*slow_dev + fast_dev + 100.0*err_dev;
							//objective_function_value += 100.0*slow_dev + fast_dev + 100.0*err_dev;
							objective_function_value += 100.0*problem_array[i]->slow_function_evals + problem_array[i]->fast_function_evals + 100.0*err_dev;
						} else {
