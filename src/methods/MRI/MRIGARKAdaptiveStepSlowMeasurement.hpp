...
public:
	...
	MRIGARKAdaptiveStepSlowMeasurement(MRICoefficients* coeffs_, SingleRateMethodCoefficients* inner_coeffs_, Problem* problem, int problem_dimension_, WeightedErrorNorm* err_norm_) :
	...
	dirk(inner_coeffs_, &(MRIGARKAdaptiveStepSlowMeasurement::explicit_solve_problem), problem_dimension_, err_norm_),
			...
			for(std::vector<int> stage_group : coeffs->stage_groups) {
				...
				for(int stage_group_index=0; stage_group_index<stage_group.size(); stage_group_index++) {
					if (status == 0) {
						...
						// If not last stage, store function evals of the stage FIXFIXFIXFIX
