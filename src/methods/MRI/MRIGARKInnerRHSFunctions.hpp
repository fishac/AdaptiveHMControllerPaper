...
public:
	...
	void explicit_solve_rhs_mrigark(double theta, vec* v, vec* f) {
		...
		for(int sub_stage_index=0; sub_stage_index<stage_index; sub_stage_index++) {
			...
			f_temp *= gamma(stage_index + embedding_shift, sub_stage_index, tau);
			(*f) += f_temp;
		}
		(*f) /= delta_c;
	...
	void explicit_solve_rhs_imexmrigark(double theta, vec* v, vec* f) {
		...
		for(int sub_stage_index=0; sub_stage_index<stage_index; sub_stage_index++) {
			...
			f_temp *= omega(stage_index + embedding_shift, sub_stage_index, tau);
			(*f) += f_temp;
		}
		(*f) /= delta_c;
		problem->fast_rhs(theta, v, &f_temp);
	...
	void explicit_solve_rhs_mrexp(double theta, vec* v, vec* f) {
		...
		if (stage_index > 1) {
			...
			for(int sub_stage_index=0; sub_stage_index<stage_index-1; sub_stage_index++) {
				...
				f_temp *= gamma_mrexp(stage_index + embedding_shift,sub_stage_index,theta/H);
	...
	void explicit_solve_rhsjacobian(double theta, vec* v, mat* jac) {
		...
		if (method_type == 0) {
			explicit_solve_rhsjacobian_mrigark(theta,v,jac);
		} else if (method_type == 1) {
			explicit_solve_rhsjacobian_mrexp(theta,v,jac);
		}
	}
	
	void explicit_solve_rhsjacobian_mrigark(double theta, vec* v, mat* jac) {
		problem->fast_rhsjacobian(theta, v, &jac_temp);
		*jac = jac_temp;
	}
	
	void explicit_solve_rhsjacobian_mrexp(double theta, vec* v, mat* jac) {
		problem->linear_rhsjacobian(theta, v, &jac_temp);
		*jac = jac_temp;
	}
	...
	void implicit_set_previous_terms_imexmrigark() {
		for(int sub_stage_index=0; sub_stage_index<stage_index; sub_stage_index++) {
			...
			f_temp = mri_eval_stages2.col(sub_stage_index);
			f_temp *= omega_bar(stage_index + embedding_shift, sub_stage_index);
			implicit_previous_terms += f_temp;
		}
	}
	...
	void erk_step_mrigark(vec* y_prev, vec* f) {
		for(int sub_stage_index=0; sub_stage_index<stage_index; sub_stage_index++) {
			...
			f_temp2 *= gamma_bar(stage_index + embedding_shift, sub_stage_index);
			f_temp += f_temp2;
		}
		(*f) = *y_prev + H*f_temp;
	}
	...
	void erk_step_imexmrigark(vec* y_prev, vec* f) {
		for(int sub_stage_index=0; sub_stage_index<stage_index; sub_stage_index++) {
			...
			f_temp2 *= omega_bar(stage_index + embedding_shift, sub_stage_index);
			f_temp += f_temp2;
		}
		(*f) = *y_prev + H*f_temp;
	}
